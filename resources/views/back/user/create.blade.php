		    ...
		    <form class="col s12" method="post" action="{{ url('admin/user/') }}">
		      ...
		      <div class="row">
		        <div class="input-field col s4">
		          <input id="user_name" name="name" type="text" class="validate" value="{{old('name')}}">
		          <label for="user_name">Name</label>
         			@include('partials.flash-error-field', ['field' => 'name'])
		        </div>
		        <div class="input-field col s4">
		          <input id="user_email" name="email" type="email" class="validate" value="{{old('email')}}">
				...
				<div class="input-field col s4">
					<select name="role"  >
						...
						<option  value="administrator"  {{ selected_fields('administrator' , old('role'), 'selected') }} >Administrator</option>
						<option  value="editor" {{ selected_fields(  'editor' , old('role'), 'selected') }} >Editor</option>
					</select>
					<label>Role</label>
